...
uint32_t SimpleIdMapper::GetNodeIdForThread(uint32_t tid) { return tid / this->kMaxThreadsPerNode; }
...
uint32_t SimpleIdMapper::AllocateWorkerThread(uint32_t node_id) {
  ...
  while (i + this->kMaxBgThreadsPerNode < kMaxThreadsPerNode) {
    if (this->node2worker_[node_id].find(node_id * kMaxThreadsPerNode + i + this->kMaxBgThreadsPerNode) ==
        this->node2worker_[node_id].end()) {
...
std::vector<uint32_t> SimpleIdMapper::GetServerThreadsForId(uint32_t node_id) { return this->node2server_[node_id]; }
